...
public class Examples {
	public static void main(String[] args) {
		...
		IntStream.range(1, 10) // gives the stream of integers from 1 to 9
		...
		System.out.println(IntStream.range(1, 5).sum() // this will return the sum from 1 to 4 and return the result to
														// println() method which will print it
		);
		...
		Stream.of("Bob", "Allen", "Cathy") // making a stream of strings
				...
				.ifPresent(System.out::println); // prints the element if present. If the optional object is empty then
													// this method doesn't gets executed
		...
		Arrays.stream(new int[] { 1, 2, 3, 4, 5, 6 }).map(x -> x * x) // squaring each integer
				.average() // taking average of all the elements
				.ifPresent(System.out::println);

		// stream from list, filter and print
		List<String> listNames = Arrays.asList(names);
		listNames.stream().map(String::toLowerCase).filter(x -> x.startsWith("a")).forEach(System.out::println);
		...
		map = studentsStream.map(x -> x.split(",")).collect(Collectors.toMap(x -> Integer.parseInt(x[0]), x -> x[1]));
		...
		IntSummaryStatistics summary = IntStream.of(3, 6, 9, 13, 23, 9).summaryStatistics();
